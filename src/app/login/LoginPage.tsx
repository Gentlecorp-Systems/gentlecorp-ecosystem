...
const handleLogin: (
  ...
  formData?: FormData
) => Promise<AuthResponse> | SignInResponse = async (provider, formData) => {
  if (provider.id === "credentials") {
    return new Promise(async (resolve) => {
      try {
        ...
        // Hier wird das Promise mit dem Ergebnis aufgelöst
        resolve(result);
      } catch (error) {
        resolve({ ok: false, error } as SignInResponse);
      }
    });
  }
  ...
  return (
    <AppProvider branding={BRANDING} theme={theme}>
      <Box
        sx={{
          minHeight: "100vh",
          background: "linear-gradient(135deg, #F8F8FC, #6A4BBC, #4E3792, #000)",
